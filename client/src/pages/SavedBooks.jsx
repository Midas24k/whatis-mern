...
const SavedBooks = () => {
  const { loading, data } = useQuery(GET_ME); // Use useQuery hook to execute GET_ME query
  ...
  const handleDeleteBook = async (bookId) => {
    ...
    try {
      const { data } = await removeBookMutation({
        variables: { bookId }, // Pass bookId as variable to the mutation
      });
  ...
  const userData = data ? data.me : {}; // Extract user data from the query result
